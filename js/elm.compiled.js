...
Elm.Main.make = function (_elm) {
   ...
   $Time = Elm.Time.make(_elm);
   var _op = {};
   var clearBlue = A4($Color.rgba,111,111,111,0.5);
   var clearGrey = A4($Color.rgba,111,255,111,0.6);
   var makePlayer = F2(function (x,y) {
      return A3($Graphics$Collage.collage,
      1600,
      800,
      _U.list([A2($Graphics$Collage.move,
              {ctor: "_Tuple2",_0: -700,_1: $Basics.toFloat(y)},
              A2($Graphics$Collage.filled,clearGrey,A2($Graphics$Collage.ngon,100,40)))
              ,A2($Graphics$Collage.move,
              {ctor: "_Tuple2",_0: $Basics.toFloat(x),_1: 0},
              A2($Graphics$Collage.rotate,90,A2($Graphics$Collage.filled,clearBlue,A2($Graphics$Collage.ngon,4,50))))]));
   });
   var view = F2(function (address,_p0) {    var _p1 = _p0;return A2($Html.div,_U.list([]),_U.list([$Html.fromElement(A2(makePlayer,_p1.x,_p1.y))]));});
   var update = F2(function (action,model) {
      var _p2 = action;
      switch (_p2.ctor)
      {case "NoOp": return {ctor: "_Tuple2",_0: model,_1: $Effects.none};
         case "UpdateTime": var obstacle = _U.eq(model.x,-800) ? 0 : model.x - 20;
           var isJumping = model.isJumping && (_U.cmp(model.y,0) > -1 && _U.cmp(model.y,300) < 0) ? true : false;
           var jump = isJumping ? 20 : _U.cmp(model.y,0) > 0 ? -30 : 0;
           var newModel = _U.update(model,{x: obstacle,y: model.y + jump,isJumping: isJumping});
           return {ctor: "_Tuple2",_0: newModel,_1: $Effects.none};
         default: var newModel = _U.update(model,{isJumping: _p2._0 || model.isJumping});
           return {ctor: "_Tuple2",_0: newModel,_1: $Effects.none};}
   });
   var NoOp = {ctor: "NoOp"};
   var SpacebarDown = function (a) {    return {ctor: "SpacebarDown",_0: a};};
   var UpdateTime = function (a) {    return {ctor: "UpdateTime",_0: a};};
   var inputs = _U.list([A2($Signal.map,UpdateTime,$Time.fps(30)),A2($Signal.map,SpacebarDown,$Keyboard.space)]);
   var Model = F3(function (a,b,c) {    return {x: a,y: b,isJumping: c};});
   var initModel = A3(Model,0,0,false);
